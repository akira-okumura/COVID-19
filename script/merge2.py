...
# A new version for limited Nagoya data format
# $ cd CTV
# $ mmdd=0807; ../script/merge2.py Aichi_official_$mmdd.tsv Toyota_official_$mmdd.tsv Okazaki_official_$mmdd.tsv Toyohashi_official_$mmdd.tsv Nagoya_official_$mmdd.tsv > merged_$mmdd.tsv
...
def tsv(fname):
    ...
    for line in f.readlines():
        ...
        try:
            ...
            try:
                num, age, sex, city, level, note = line[:-1].split('\t')
                abroad, symp_date, pos_date = '', '', ''
            except:
                import sys
        ...
        if len(pub_date) == 4:
            ...
            y, m, d = int(pub_date[:4]), int(pub_date[4:6]), int(pub_date[6:])
        ...
        note = re.sub('名古屋市発表([,\d]*)、名古屋市発表([,\d]*)', '名古屋市発表\\1,\\2', note)
        note = re.sub('名古屋市発表([,\d]*)。豊田市発表([,\d]*)。', '名古屋市発表\\1,豊田市発表\\2', note)
        note = re.sub('岐阜県内([,\d]*)。名古屋市発表([,\d]*)。', '岐阜県内\\1,名古屋市発表\\2', note)
        note = re.sub('名古屋市発表([,\d]*)。岐阜県内([,\d]*)。', '名古屋市発表\\1,岐阜県内\\2', note)
        note = re.sub('愛知県内([,\d]*)。岐阜県内([,\d]*)。', '愛知県内\\1,岐阜県内\\2', note)
